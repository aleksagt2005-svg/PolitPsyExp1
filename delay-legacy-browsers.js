...
let expName = 'delay';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'shahabbas.jpeg', 'path': 'shahabbas.jpeg'},
    {'name': 'ivaniii.jpg', 'path': 'ivaniii.jpg'},
    {'name': 'willgelmzavoev.jpg', 'path': 'willgelmzavoev.jpg'},
    {'name': 'cisi.png', 'path': 'cisi.png'},
    {'name': 'mariatudor.jpeg', 'path': 'mariatudor.jpeg'},
    {'name': 'lakshmibai.jpeg', 'path': 'lakshmibai.jpeg'},
    {'name': 'hatshepsut.jpeg', 'path': 'hatshepsut.jpeg'},
    ...
    {'name': 'nurdjuhan.jpeg', 'path': 'nurdjuhan.jpeg'},
...
async function experimentInit() {
  ...
  fetch('https://pipe.jspsych.org/api/data', {
      ...
      headers: {
       ...
       body: JSON.stringify({
          experimentID: 'ZKQhwJkgegrj', // * DATAPIPE EXP ID*
          filename: filename, 
          data: data,
       }),
...
function softRoutineBegin(snapshot) {
  return async function () {
    ...
    softComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function questsoftRoutineBegin(snapshot) {
  return async function () {
    ...
    questsoftComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
